...
use candle_transformers::{generation::LogitsProcessor, models::qwen3::{Config, ModelForCausalLM}};
...
pub fn train_main() -> Result<()> {
    ...
    let tokenizer = Tokenizer::from_file("/Users/x/Rust/DeepLearning/rust-candle/src/assets/tokenizer.json")
        .map_err(|e| Error::Msg(format!("tokenizer from file error:{}", e)))?;
    let batch_size = 2;
    let config = LLMConfig::default()?;

    let val_txt = read_text("/Users/x/Rust/DeepLearning/rust-candle/src/assets/sub_wiki_0_99.txt");
    ...
    let train_txt = read_text("/Users/x/Rust/DeepLearning/rust-candle/src/assets/sub_wiki_0_99.txt");
...
pub fn test_qwen3() -> Result<()> {
    let tokenizer_file = "models/Qwen3-0.6B/config.json";
    let weight_file = "models/Qwen3-0.6B/model.safetensors";
    let config_file = "models/Qwen3-0.6B/config.json";
